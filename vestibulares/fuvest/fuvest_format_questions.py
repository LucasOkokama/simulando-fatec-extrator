    ...
    with open('gemini-api-key.txt', 'r', encoding='utf-8') as api_key_txt:
      ...
      for num_questao in range(1, 91):
        try:
          ...
          print(f"> Criando JSON da questao {num_questao} ({anoDaProva})")
          ...
          print(f"> Identificando o texto de suporte da questao {num_questao} ({anoDaProva})")
          # Retorna o custo de Token da requisição
          ...
          print(f"> Identificando disciplina da questao {num_questao} ({anoDaProva})")
          # Retorna o custo de Token da requisição
          ...
          path_json_formatted = os.path.join(dir_questions_number_json, f"fuvest_questao{num_questao}.json")
